...
function persistCredentials(tokens: any) {
  ...
  childProcess.exec('gcloud config set account ' + payload.email, {env: process.env}, function(err, stdout, stderr) {
    if (err) {
...
export function handleAuthFlow(request: http.ServerRequest, response: http.ServerResponse, parsed_url: any): boolean {
  ...
  } else if (!oauth2Client && process.env.DATALAB_ENV == 'local' && !fs.existsSync(userCredFile)) {
    // Do auth.
